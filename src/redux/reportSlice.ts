...
const makeRequest = async (url: string, method: 'get' | 'post', data?: Goal) => {
    try {
...
export const getSummaryReport = createAsyncThunk("report/summary", async (data: { startDate: string, endDate: string , type: number}) => {
    return await makeRequest(`/report/expense-summary?startDate=${data.startDate}&endDate=${data.endDate}&type=${data.type}`, 'get');
});
export const getGoalReport = createAsyncThunk("report/goals", async (goalId: number) => {
